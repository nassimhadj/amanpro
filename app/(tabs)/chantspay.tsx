...
export default function chantspay({ navigation }) {
  ...
  useEffect(() => {
    ...
    const fetchpayChantiers = async () => {
      try {
        ...
        const payChantiers = data.filter(rdv => rdv.status === 'attente de payement');
        console.log('Filtered en cours chantiers:', payChantiers);
        
        setChantiers(payChantiers);
      } catch (error) {
        ...
        {filteredChantiers.length === 0 ? (
          <View style={styles.noResultsContainer}>
            <Text style={styles.grdtext}>
              Aucun chantier en attente de payement trouvé
            </Text>
          </View>
        ) : (
            ...
            <View key={chantier._id} style={styles.column}>
              ...
              <View style={styles.vbutton}>
                <TouchableOpacity
                  style={styles.button}
                  onPress={() => 
                    navigation.navigate("chantpay", { chantier: chantier })
                  }
                >
